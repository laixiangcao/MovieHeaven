package com.jiumeng.movieheaven.provider;
...
public class ProcessData {
     ...
     * @return 电影集合
     */
    public static List<MovieEntity> parsePageData(String text) {
        String contents;
        // 截取网页主要文本文本
        ...
        for (String s : split) {
            ...
            if (index3 != -1 && index4 != -1) {
                movieDao.url = content.substring(index3 + 9, index4 + 5);
                movieDao.id = TextUtils.textToMD5(movieDao.url);
            } else {
                movieDao.url = "";
                movieDao.id = "";
            }
...
//            }
            ...
            if (isLoadAll){
                ...
                //id
                if (movie.url!=null){
                    movie.id= TextUtils.textToMD5(movie.url);
                }else {
                    movie.id="";
                }
    ...
    public static List<List<String>> getHomeUrl(String text) {
        List<List<String>> homeUrlList = new ArrayList<>();
        ...
        for (String subString : subUrls) {
            ...
            while (m.find()) {
                List.add(m.group().replace("href='/",""));
            }
            homeUrlList.add(List);
        }
        return homeUrlList;
    }
